...
// delivers the third argument, producing the silent failure "rc=0 yet every field is 0".
...
#define TIOCGWINSZ_DARWIN 0x40087468UL

int mdpx_winsize(int fd, unsigned short *out) {
  unsigned short ws[4] = {0, 0, 0, 0};
  if (ioctl(fd, TIOCGWINSZ_DARWIN, ws) != 0) return -1;
  out[0] = ws[0];
  out[1] = ws[1];
  out[2] = ws[2];
  out[3] = ws[3];
