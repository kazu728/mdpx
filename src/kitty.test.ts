...
describe("imageId", () => {
  test("gen*ID_STRIDE + tileIndex, always at least 1", () => {
    expect(imageId(1, 0)).toBe(ID_STRIDE);
    expect(imageId(2, 5)).toBe(2 * ID_STRIDE + 5);
...
describe("transmit", () => {
  test("splits at the 4096 boundary and continues the m flag", () => {
    const b64 = "A".repeat(CHUNK_SIZE * 2 + 10);
    const apcs = parseApc(transmit(imageId(3, 1), b64));
    expect(apcs).toHaveLength(3);
    expect(apcs[0]!.payload.length).toBe(CHUNK_SIZE);
    expect(apcs[1]!.payload.length).toBe(CHUNK_SIZE);
...
describe("place", () => {
  test("the source rect and display cell parameters", () => {
    const s = place({ id: 2053, srcX: 0, srcY: 120, srcW: 3024, srcH: 620, cols: 216, rows: 20 });
    const [apc] = parseApc(s);
    expect(keys(apc!.control)).toMatchObject({
      a: "p", i: "2053", p: String(PLACEMENT_ID),
...
describe("delete", () => {
  test("deletePlacement removes the placement only (d=i)", () => {
    expect(keys(parseApc(deletePlacement(2053))[0]!.control)).toMatchObject({
      a: "d", d: "i", i: "2053", p: String(PLACEMENT_ID),
    });
  });
