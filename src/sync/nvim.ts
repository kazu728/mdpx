// Best-effort: failures are silent no-ops and never block rendering.
...
/** macOS truncates unix socket paths at 104 bytes. */
...
/** Path and line go through _A, never mixed into the chunk syntax. */
...
/** Spawning the CLI keeps the helper stateless (no msgpack dependency). */
...
/** Picks up both layouts: the root and one level below. */
...
export class NvimCursor {
  ...
  /**
   * Equal lines are still sent; the nvim cursor may have moved independently.
   */
  ...
  // No loop: a line arriving mid-flight re-arms and waits for the next settle.
  ...
  // buffer-missing/window-missing just means "not the target session".
  private async trySend(socket: string, line: number): Promise<boolean> {
    if (this.aborter.signal.aborted) return true;
