...
const waitFor = async (cond: () => boolean, n = 500) => {
  for (let i = 0; i < n && !cond(); i++) await sleep(1);
};
...
async function makePipeline(chrome: unknown, opts?: { cols?: number; write?: (s: string) => void; shuttingDown?: boolean; md?: string }) {
